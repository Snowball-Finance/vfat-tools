...
async function main() {
  ...
  const approveStable4 = async function () {
    return tundraContract_approve(TUNDRA_ABI, TUNDRA_ADDRESS, STABLE_4_ADDRESS, App)
  }
  const approveLP = async function () {
    return tundraContract_approve(TUNDRA_ABI, TUNDRA_ADDRESS, S4D_ADDRESS, App)
  }
  ...
  const revokeStable4 = async function () {
    return tundraContract_revoke(TUNDRA_ABI, TUNDRA_ADDRESS, STABLE_4_ADDRESS, App)
  }
  const revokeLP = async function () {
    return tundraContract_revoke(TUNDRA_ABI, TUNDRA_ADDRESS, S4D_ADDRESS, App)
  }
  const depositStables = async function () {
    return tundraContract_deposit(TUNDRA_ABI, TUNDRA_ADDRESS, STABLE_1_ADDRESS, STABLE_2_ADDRESS, STABLE_3_ADDRESS, STABLE_4_ADDRESS App)
  }
  const withdrawLP = async function () {
    return tundraContract_withdraw(TUNDRA_ABI, TUNDRA_ADDRESS, S4D_ADDRESS, TUNDRA_ADDRESS, App)
  }
  // Tokens & contracts
  ...
  const STABLE_4_TOKEN = new ethers.Contract(STABLE_4_ADDRESS, ERC20_ABI, signer);
  const S4D_TOKEN = new ethers.Contract(S4D_ADDRESS, ERC20_ABI, signer);
  ...
  const s4_balance = await STABLE_4_TOKEN.balanceOf(App.YOUR_ADDRESS);
  ...
  const s4_balance_formatted = s4_balance/1e18 > .001 ? (s4_balance/1e18 - .001).toFixed(3) : 0;
  ...
  $('#token_4_balance').html(`${s4_balance_formatted}`);
  $('#withdraw_balance').html(`${(S4D_balance/1e18).toFixed(3)}`);
  ...
  const s4_supply = await STABLE_4_TOKEN.balanceOf(TUNDRA_ADDRESS);
  const combined_supply = s1_supply / 1e6 + s2_supply / 1e18 + s3_supply / 1e18 + s4_supply / 1e18;
  ...
  const s4_supply_percentage = (s4_supply / 1e18 / combined_supply * 100) || 0;
  const S4D_supply = await S4D_TOKEN.totalSupply();
  const user_percentage = ((S4D_balance / 1e18) / (S4D_supply / 1e18) * 100) || 0;
  const S4D_ratio = combined_supply / (S4D_supply / 1e18);
  console.log("S4D_ratio:", S4D_ratio);
  ...
  const totalStakedS4D = await S4D_TOKEN.balanceOf(ICEQUEEN_ADDR)
  const pool7tvl = totalStakedS4D / 1e18;
  ...
  const t4_supply_display = new Intl.NumberFormat('en-US').format((s4_supply / 1e18).toFixed(2));
  ...
  $("#withdraw_open_btn").click(function() {
    loadWithdrawModal(TUNDRA_CONTRACT, S4D_TOKEN, App, true);
  ...
  $("#withdraw_percentage, #radio-withdraw-combo, #radio-withdraw-usdt, #radio-withdraw-busd, #radio-withdraw-dai").change(function() {
    updateWithdrawAmount(TUNDRA_CONTRACT, S4D_TOKEN, App);
  });
...
const updateWithdrawAmount = async function(TUNDRA_CONTRACT, S4D_TOKEN, App){
  ...
  let calculatedWithdraw = S4D_balance.mul(withdrawPercentage * 100 || 0).div(100).div(100);
...
const loadWithdrawModal = async function(TUNDRA_CONTRACT, S4D_TOKEN, App){
...
const loadDepositModal = async function(TUNDRA_CONTRACT, App, S4D_ratio, STABLE_1_TOKEN, STABLE_2_TOKEN, STABLE_3_TOKEN, TUNDRA_ADDRESS){
  ...
  console.log("S4D_ratio deposit modal:", S4D_ratio);
  ...
  $("#receiving_ratio").html(((S4D_ratio || 1)).toFixed(6));
  $("#receiving_usd").html(`$${((S4D_ratio || 1) * minToMint / 1e18).toLocaleString()}`);

  // premium & fee
  const totalAmount = Number(s1_input) + Number(s2_input) + Number(s3_input)
  const difference = (minToMint / 1e18 * (S4D_ratio || 1)) - totalAmount;
...
const tundraContract_withdraw = async function (chefAbi, chefAddress, S4D_token, tundra_address, App) {
  $("#withdraw_confirm_btn").prop('disabled', true);
  const signer = App.provider.getSigner()
  console.log(signer)

  //Tokens
  const S4D_TOKEN = new ethers.Contract(S4D_token, ERC20_ABI, signer)
  const CHEF_CONTRACT = new ethers.Contract(chefAddress, chefAbi, signer)
  const S4D_allowance = await S4D_TOKEN.allowance(App.YOUR_ADDRESS, tundra_address)
  const withdrawPercentage = $("#withdraw_percentage").val();
  const comboChecked = $("#radio-withdraw-combo").is(':checked');
  ...
  if (calculatedWithdraw == 0 || S4D_allowance == 0){
